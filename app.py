...
if os.path.exists('/data'):
    ...
    if os.path.exists(original_servers) and not os.path.islink(original_servers):
        try:
            ...
            if not os.listdir(PERSISTENT_SERVERS):
                os.rmdir(original_servers)
                os.symlink(PERSISTENT_SERVERS, original_servers)
                print("[*] Symlinked /servers to persistent storage")
            else:
                print("[*] Kept local /servers (persistent copy already holds data) - no data destroyed")
        except Exception as e:
        ...
        try:
            for item in os.listdir(original_uploads):
                ...
                if os.path.isdir(s_path):
                    ...
                    shutil.copy2(s_path, d_path)
            if not os.listdir(PERSISTENT_UPLOADS):
                os.rmdir(original_uploads)
                os.symlink(PERSISTENT_UPLOADS, original_uploads)
                print("[*] Symlinked /uploads to persistent storage")
            else:
                print("[*] Kept local /uploads - no data destroyed")
        except Exception as e:
        ...
        try:
            for item in os.listdir(original_avatars):
                ...
                if os.path.isdir(s_path):
                    ...
                    shutil.copy2(s_path, d_path)
            if not os.listdir(PERSISTENT_AVATARS):
                os.rmdir(original_avatars)
                os.symlink(PERSISTENT_AVATARS, original_avatars)
                print("[*] Symlinked avatars to persistent storage")
            else:
                print("[*] Kept local avatars dir - no data destroyed")
        except Exception as e:
        ...
        try:
            for item in os.listdir(original_branding):
                ...
                if os.path.isdir(s_path):
                    ...
                    shutil.copy2(s_path, d_path)
            if not os.listdir(PERSISTENT_BRANDING):
                os.rmdir(original_branding)
                os.symlink(PERSISTENT_BRANDING, original_branding)
                print("[*] Symlinked branding to persistent storage")
            else:
                print("[*] Kept local branding dir - no data destroyed")
        except Exception as e:
...
# Database connection helper — works in both Flask request context AND background threads
def get_db():
    """Return a DB connection. Inside a Flask request context uses flask.g for connection pooling.
    Outside a request context (background threads, startup) falls back to a direct sqlite3 connection."""
    try:
        ...
        # Called from background thread or startup — create a standalone connection
        return _connect_db()

def _connect_db():
    """Create a SQLite connection tuned for concurrent web + background-thread access."""
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        admin_email = os.environ.get('ADMIN_EMAIL', 'adminrifat@gmail.com')
    ...
    primary_super_admins = ['admin@hostx.io', 'admin@hostx.vip', 'adminrifat@gmail.com', 'admin']
...
SERVER_START_TIMES['boot'] = time.time()
...
def _background_threads_enabled():
    return os.environ.get('DISABLE_BACKGROUND_THREADS', '').strip().lower() not in ('1', 'true', 'yes')
...
def is_user_super_admin(user):
    if not user:
        return False
    keys = user.keys()
    role = user['role'] if ('role' in keys and user['role']) else None
    is_super = user['is_super_admin'] if ('is_super_admin' in keys and user['is_super_admin'] is not None) else 0
...
def is_user_admin(user):
    ...
    keys = user.keys()
    role = user['role'] if ('role' in keys and user['role']) else None
    is_admin_val = user['is_admin'] if ('is_admin' in keys and user['is_admin'] is not None) else 0
    if role in ('admin', 'super_admin') or str(is_admin_val) in ('1', 'true', 'True'):
        return True
    return False

def has_admin_permission(user, permission_name):
    """Fail closed: an admin with no explicit permission list gets no permissions."""
...
def check_maintenance_and_auth():
    # Resolve the public site URL from env, stored setting, or the incoming request
    try:
        url_root = APP_URL
        if not url_root:
            url_root = (get_setting('site_url', '') or '').strip().rstrip('/')
        if not url_root:
...
def check_server_ownership(server_id, user_id=None):
    ...
    if curr_user and is_user_admin(curr_user):
        return server
...
# Bidirectional package and import mappings
...
IMPORT_TO_PACKAGE_MAP = {
    ...
    'Crypto': {'pkg': 'pycryptodome', 'version': '3.23.0'},
    'cryptodome': {'pkg': 'pycryptodomex', 'version': '3.23.0'},
    'pycryptodome': {'pkg': 'pycryptodome', 'version': '3.23.0'},
...
def is_package_or_import_installed(server_dir, import_or_pkg_name):
    ...
    # Collect all possible aliases (import name, package name, dist-info names)
    names_to_check = {import_or_pkg_name.lower()}
    if import_or_pkg_name.lower() in PACKAGE_TO_IMPORT_MAP:
        names_to_check.add(PACKAGE_TO_IMPORT_MAP[import_or_pkg_name.lower()].lower())
    if import_or_pkg_name in IMPORT_TO_PACKAGE_MAP:
        names_to_check.add(IMPORT_TO_PACKAGE_MAP[import_or_pkg_name]['pkg'].lower())
    for imp, val in IMPORT_TO_PACKAGE_MAP.items():
        if val['pkg'].lower() == import_or_pkg_name.lower():
            names_to_check.add(imp.lower())

    # Check direct directory / dist-info existence first for speed
    if os.path.exists(packages_dir):
        try:
            items = os.listdir(packages_dir)
            for item in items:
                item_raw = item.lower()
                item_norm = item_raw.replace('-', '_')
                for n in names_to_check:
                    n_norm = n.replace('-', '_')
                    if item_raw == n or item_norm == n_norm:
                        return True
                    if item_raw.startswith(f"{n}-") or item_norm.startswith(f"{n_norm}_") or item_raw.startswith(f"{n}."):
                        return True
        except Exception:
            pass

    # Test via Python subshell import check with server's PYTHONPATH
    import_mod = PACKAGE_TO_IMPORT_MAP.get(import_or_pkg_name.lower(), import_or_pkg_name)
    test_code = f"import {import_mod}"
    env = os.environ.copy()
    paths = [p for p in [server_dir, packages_dir, env.get('PYTHONPATH', '')] if p]
    ...
    for alt in names_to_check:
        if alt != import_mod.lower():
            try:
                res = subprocess.run(
...
def scan_and_analyze_project(server_dir, server_id=None):
    ...
    for pkg_name, ver in req_packages.items():
        ...
        import_name = PACKAGE_TO_IMPORT_MAP.get(pkg_name.lower())
        if not import_name:
    ...
    for imp_name in extracted_imports:
        ...
        pkg_name = mapping.get('pkg')
        if not pkg_name:
            for p, im in PACKAGE_TO_IMPORT_MAP.items():
                if im.lower() == imp_name.lower():
                    pkg_name = p
                    break
        if not pkg_name:
            pkg_name = imp_name
        ...
        # Check if already added via requirements.txt or matching import
        existing_key = None
        for k in required_packages_dict.keys():
            if k.lower() == pkg_name.lower() or required_packages_dict[k].get('import_name', '').lower() == imp_name.lower():
                existing_key = k
                break
...
def background_process_monitor():
    """Continuously monitors running server processes, catches runtime dependency errors,
    and automatically restarts servers that have auto_restart enabled."""
    # Servers queued for auto-restart: {server_id: restart_at_timestamp}
    _restart_queue = {}
    while True:
        try:
            ...
            # Handle pending auto-restarts
            now_ts = time.time()
            for s_id, restart_at in list(_restart_queue.items()):
                if now_ts >= restart_at and s_id not in RUNNING_PROCESSES:
                    _restart_queue.pop(s_id, None)
                    try:
                        print(f"[HostX AutoRestart] Restarting server {s_id}...", flush=True)
                        with app.app_context():
                            start_server_process(s_id)
                    except Exception as re_err:
                        print(f"[HostX AutoRestart] Failed to restart server {s_id}: {re_err}", flush=True)
                ...
                if proc.poll() is not None:
                    ...
                    try:
                        ...
                        if srv:
                            ...
                            # Check if auto_restart is enabled for this server
                            if srv['auto_restart'] == 1:
                                exp_str = srv['expires_at'] if srv['expires_at'] else ''
                                try:
                                    if isinstance(exp_str, str):
                                        exp_dt = datetime.strptime(exp_str.split('.')[0], '%Y-%m-%d %H:%M:%S')
                                    else:
                                        exp_dt = exp_str
                                    if exp_dt > datetime.now() and server_id not in _restart_queue:
                                        should_auto_restart = True
                                        conn.execute(
                                            "INSERT INTO server_logs (server_id, level, message) VALUES (?, ?, ?)",
                                            (server_id, 'WARNING', f"Auto-restart enabled — scheduling restart in 5 seconds (exit code {exit_code}).")
                                        )
                                        conn.commit()
                                except Exception:
                                    pass
                    ...
                    if should_auto_restart and server_id not in _restart_queue:
                        _restart_queue[server_id] = time.time() + 5  # restart after 5 seconds
        except Exception:
            pass
...
# ---- Auto-Boot: start all servers with auto_restart=1 when HostX launches ----
def boot_auto_servers():
    """On host startup, automatically start all active servers with auto_restart enabled.
    Runs in a background thread so Flask can start serving requests immediately."""
    time.sleep(3)  # Wait a moment for Flask to fully initialize
    print("[HostX AutoBoot] Scanning for servers to auto-start...", flush=True)
    conn = None
    try:
        conn = _connect_db()
        now_iso = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        servers = conn.execute(
            "SELECT id, user_id, name FROM servers WHERE auto_restart = 1 AND status != 'expired' AND expires_at > ?",
            (now_iso,)
        ).fetchall()
        conn.close()
        conn = None
        ...
        if not servers:
            print("[HostX AutoBoot] No servers configured for auto-start.", flush=True)
            return

        for srv in servers:
            s_id = srv['id']
            s_name = srv['name']
            if s_id in RUNNING_PROCESSES:
                print(f"[HostX AutoBoot] Server '{s_name}' (ID:{s_id}) already running — skipping.", flush=True)
                continue
            print(f"[HostX AutoBoot] Auto-starting server '{s_name}' (ID:{s_id})...", flush=True)
            try:
                with app.app_context():
                    ok, msg, _, _ = start_server_process(s_id)
                    if ok:
                        print(f"[HostX AutoBoot] [OK] Server '{s_name}' (ID:{s_id}) started successfully.", flush=True)
                    else:
                        print(f"[HostX AutoBoot] [WARN] Server '{s_name}' (ID:{s_id}) failed to start: {msg}", flush=True)
            except Exception as boot_err:
                print(f"[HostX AutoBoot] [ERR] Error starting server '{s_name}' (ID:{s_id}): {boot_err}", flush=True)
    except Exception as e:
        print(f"[HostX AutoBoot] [ERR] Boot scan failed: {e}", flush=True)
    finally:
        if conn is not None:
            try:
                conn.close()
...
if _background_threads_enabled():
    _autoboot_thread = threading.Thread(target=boot_auto_servers, daemon=True)
    _autoboot_thread.start()
    print("[*] HostX AutoBoot engine started in background thread.", flush=True)
...
def start_server_process(server_id):
    with START_LOCK:
        return _start_server_process_unlocked(server_id)

def _start_server_process_unlocked(server_id):
    db = get_db()
    server = db.execute("SELECT * FROM servers WHERE id = ?", (server_id,)).fetchone()
    if not server:
        return False, "Server not found", []
    
    # 0. Never run the same server twice (protects against a second gunicorn worker
    #    or a double-click racing the background monitor)
    if server_id in RUNNING_PROCESSES:
        existing = RUNNING_PROCESSES[server_id]
    ...
    if os.path.isfile(req_path):
        if not scan['ready'] or not os.path.exists(local_pkg_dir) or not os.listdir(local_pkg_dir):
                    ...
                    for line in proc_pip.stdout:
                        ...
                        if line_str:
                            ...
                            with open(log_file_path, 'a', encoding='utf-8') as lf:
                                lf.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] [pip] {line_str}\n")
                proc_pip.wait()
            ...
            write_server_log(server_id, 'INFO', "requirements.txt detected. All dependencies verified in sandbox.")
    ...
    try:
        ...
        if os.path.exists(local_pkg_dir):
            env['PYTHONPATH'] = os.pathsep.join([p for p in [server_dir, local_pkg_dir, env.get('PYTHONPATH', '')] if p])
        else:
            env['PYTHONPATH'] = os.pathsep.join([p for p in [server_dir, env.get('PYTHONPATH', '')] if p])
            
        # Cross-platform process group separation: Windows uses CREATE_NEW_PROCESS_GROUP, Unix uses os.setsid
        creationflags = 0
        preexec = None
...
def health():
    uptime = time.time() - SERVER_START_TIMES.get(0, SERVER_START_TIMES.get('boot', time.time()))
    return jsonify({
        'status': 'ok',
        'app': 'HostX VIP White',
...
def signin():
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        # Authenticate by registered email address OR username (case-insensitive)
        user = db.execute(
...
def renew_server(server_id):
    ...
    base_time = max(datetime.now(), current_exp_dt)
...
def is_safe_path(base_dir, path, follow_symlinks=True):
    """Resolve BOTH sides before comparing so symlinked storage roots (/data/servers)
    do not falsely fail, while still blocking traversal outside the server folder."""
    ...
    try:
        return os.path.commonpath((base_real, matchpath)) == base_real
    except ValueError:
        return False

MAX_EDIT_FILE_BYTES = 5 * 1024 * 1024
MAX_ARCHIVE_BYTES = 500 * 1024 * 1024
MAX_ARCHIVE_MEMBERS = 20000
ALLOWED_TEXT_EXTENSIONS = {
    '.py', '.txt', '.json', '.js', '.ts', '.html', '.htm', '.css', '.md', '.markdown',
    '.yml', '.yaml', '.ini', '.cfg', '.conf', '.env', '.xml', '.csv', '.sh', '.bash',
    '.sql', '.toml', '.gitignore', '.dockerfile', '.lock', '.properties', '.log'
}
...
def resolve_server_path(server_dir, rel_path, must_exist=True):
    """Join a user supplied relative path onto server_dir and guarantee containment."""
    rel = (rel_path or '').replace('\\', '/').strip().strip('/')
    target = os.path.normpath(os.path.join(server_dir, rel)) if rel else os.path.realpath(server_dir)
...
def is_text_editable(name):
    ext = os.path.splitext(name)[1].lower()
    if ext in ALLOWED_TEXT_EXTENSIONS:
        return True
    if os.path.basename(name).lower() in ('dockerfile', 'makefile', 'readme', 'license', 'procfile'):
        return True
    try:
...
def validate_zip_members(zf, server_dir, extract_dir):
    """Reject zip-slip, absolute paths, and oversized/bomb archives."""
    total_size = 0
    names = zf.namelist()
    if len(names) > MAX_ARCHIVE_MEMBERS:
        raise ValueError('Archive contains too many entries.')
    for member in names:
        if os.path.isabs(member) or member.startswith('/') or member.startswith('\\'):
        ...
        if total_size > MAX_ARCHIVE_BYTES:
            raise ValueError('Archive expands beyond the allowed size limit.')
        ratio = info.file_size / max(info.compress_size, 1)
        if info.file_size > 10 * 1024 * 1024 and ratio > 200:
            raise ValueError(f'Suspicious compression ratio detected for: {member}')
    return True

def iter_files_recursive(root_dir):
...
def file_manager(server_id):
    ...
    server_dir = get_server_dir(server['user_id'], server_id)
    target_dir = resolve_server_path(server_dir, req_path)
    ...
    try:
        for entry in os.scandir(target_dir):
            ...
            items.append({
                ...
                'modified': datetime.fromtimestamp(entry.stat(follow_symlinks=False).st_mtime).strftime('%Y-%m-%d %H:%M'),
                'is_zip': lower.endswith(('.zip', '.tar', '.tar.gz', '.tgz', '.rar', '.7z')),
                'is_py': lower.endswith('.py'),
                'is_text': (not is_dir) and is_text_editable(entry.path),
                'is_hidden': name.startswith('.'),
                'is_entry': name == server['entry_file']
            })
    except Exception as e:
...
def create_folder(server_id):
    ...
    server_dir = get_server_dir(server['user_id'], server_id)
    parent = resolve_server_path(server_dir, req_path, must_exist=True)
...
def create_file(server_id):
    ...
    if not parent or not os.path.isdir(parent):
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    target = os.path.join(parent, file_name)
    if not is_safe_path(server_dir, target):
        return jsonify({'success': False, 'message': 'Access denied'}), 403
...
def upload_file(server_id):
    ...
    req_path = request.form.get('path', '').strip('/')
    server_dir = get_server_dir(server['user_id'], server_id)
    target_dir = resolve_server_path(server_dir, req_path, must_exist=True)
    ...
    auto_extract = request.form.get('auto_extract', '1') not in ('0', 'false', 'False')
    ...
    for f in uploaded_files:
        if not f or not f.filename:
            continue
        # webkitdirectory uploads send "relative/dir/name.ext" in the filename
        raw_rel = f.filename.replace('\\', '/')
        rel_parts = [secure_filename(p) for p in raw_rel.split('/') if p not in ('', '.', '..')]
        if not rel_parts:
            continue
        ...
        for part in rel_parts[:-1]:
            ...
            if not is_safe_path(server_dir, dest_dir):
                return jsonify({'success': False, 'message': 'Access denied'}), 403
        os.makedirs(dest_dir, exist_ok=True)
        save_path = os.path.join(dest_dir, filename)
        if not is_safe_path(server_dir, save_path):
            return jsonify({'success': False, 'message': 'Access denied'}), 403
        f.save(save_path)
        try:
        ...
        if auto_extract and (filename.lower().endswith('.zip') or zipfile.is_zipfile(save_path)):
            try:
                with zipfile.ZipFile(save_path, 'r') as zf:
                    validate_zip_members(zf, server_dir, dest_dir)
                    zf.extractall(dest_dir)
                extracted_zip_count += 1
                if request.form.get('keep_zip', '0') not in ('1', 'true', 'True'):
                    os.remove(save_path)  # Delete zip so extracted files are shown directly
                uploaded_names.append(f"{filename} (Extracted)")
            except Exception as ze:
                # Keep the uploaded archive so nothing is lost if extraction was rejected
                count += 1
                uploaded_names.append(filename)
                write_server_log(server_id, 'WARNING', f"ZIP '{filename}' was uploaded but not extracted: {ze}")
        else:
            count += 1
            uploaded_names.append(filename)
...
def read_file_content(server_id):
    ...
    server_dir = get_server_dir(server['user_id'], server_id)
    target = resolve_server_path(server_dir, file_path)
    
    if not target or not os.path.isfile(target):
        return jsonify({'success': False, 'message': 'File not found'}), 404
    
    try:
        if os.path.getsize(target) > MAX_EDIT_FILE_BYTES:
            return jsonify({
                'success': False,
                'message': 'File is too large for the built-in editor (max 5 MB). Download it instead.'
            }), 400
...
def save_file_content(server_id):
    ...
    create_if_missing = str(payload.get('create_if_missing', '')).lower() in ('1', 'true', 'yes')
    
    server_dir = get_server_dir(server['user_id'], server_id)
    target = resolve_server_path(server_dir, file_path, must_exist=not create_if_missing)
    
    if not target:
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    if os.path.isdir(target):
        return jsonify({'success': False, 'message': 'Cannot write to a directory'}), 400
    if not create_if_missing and not os.path.isfile(target):
        return jsonify({'success': False, 'message': 'File not found'}), 404
    if isinstance(content, str) and len(content.encode('utf-8', 'ignore')) > MAX_EDIT_FILE_BYTES:
        return jsonify({'success': False, 'message': 'Content exceeds the 5 MB editor limit'}), 400
    ...
    try:
        ...
        with open(target, 'w', encoding='utf-8') as f:
            f.write(content)
        write_server_log(server_id, 'INFO', f"File saved: {os.path.relpath(target, server_dir)}")
...
def delete_file(server_id):
    ...
    target_path = (get_payload().get('path') or '').strip('/')
    server_dir = get_server_dir(server['user_id'], server_id)
    server_root = os.path.realpath(server_dir)
    target = resolve_server_path(server_dir, target_path)
    
    if not target or os.path.realpath(target) == server_root:
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    ...
    try:
        ...
        write_server_log(server_id, 'INFO', f"Deleted: {target_path}")
...
def rename_file(server_id):
    ...
    new_name = secure_filename((payload.get('new_name') or '').strip())
    
    if not new_name:
        return jsonify({'success': False, 'message': 'Invalid new name'}), 400
        
    server_dir = get_server_dir(server['user_id'], server_id)
    source = resolve_server_path(server_dir, old_path)
    if not source:
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    dest = os.path.join(os.path.dirname(source), new_name)
    if not is_safe_path(server_dir, dest):
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    ...
    try:
        ...
        if server['entry_file'] == os.path.basename(source):
            db = get_db()
            db.execute("UPDATE servers SET entry_file = ? WHERE id = ?", (new_name, server_id))
            db.commit()
        write_server_log(server_id, 'INFO', f"Renamed '{old_path}' to '{new_name}'")
        return jsonify({'success': True, 'message': f'Renamed to {new_name}'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/servers/<int:server_id>/files/copy', methods=['POST'])
@login_required
def copy_item(server_id):
    """Copy a file or folder into another folder inside the same server."""
    server = check_server_ownership(server_id)
    if not server:
        return jsonify({'success': False, 'message': 'Server not found'}), 404
        
    payload = get_payload()
    source_rel = (payload.get('path') or '').strip('/')
    dest_dir_rel = (payload.get('dest') or '').strip('/')
    overwrite = str(payload.get('overwrite', '')).lower() in ('1', 'true', 'yes')
    
    server_dir = get_server_dir(server['user_id'], server_id)
    source = resolve_server_path(server_dir, source_rel)
    if not source or source == os.path.realpath(server_dir):
        return jsonify({'success': False, 'message': 'Access denied'}), 400
    dest_dir = resolve_server_path(server_dir, dest_dir_rel, must_exist=True)
    if not dest_dir or not os.path.isdir(dest_dir):
        return jsonify({'success': False, 'message': 'Destination folder not found'}), 400
        
    final_dest = os.path.join(dest_dir, os.path.basename(source))
    if not is_safe_path(server_dir, final_dest):
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    if os.path.realpath(os.path.dirname(final_dest)) == os.path.realpath(os.path.dirname(source)):
        return jsonify({'success': False, 'message': 'Source and destination are the same folder'}), 400
    ...
    try:
        if os.path.isdir(source):
            ...
            shutil.copytree(source, final_dest, symlinks=False)
        else:
            if os.path.exists(final_dest):
                if not overwrite:
                    return jsonify({'success': False, 'message': 'Destination already exists'}), 400
        ...
        write_server_log(server_id, 'INFO', f"Copied '{source_rel}' to '{dest_dir_rel or '/'}'")
        return jsonify({'success': True, 'message': f"Copied to {dest_dir_rel or 'root'}"})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/servers/<int:server_id>/files/move', methods=['POST'])
@login_required
def move_item(server_id):
    """Move or rename a file/folder inside the same server."""
    server = check_server_ownership(server_id)
    if not server:
        return jsonify({'success': False, 'message': 'Server not found'}), 404
    ...
    server_root = os.path.realpath(server_dir)
    source = resolve_server_path(server_dir, source_rel)
    if not source or os.path.realpath(source) == server_root:
        return jsonify({'success': False, 'message': 'Access denied'}), 400
    dest_dir = resolve_server_path(server_dir, dest_dir_rel, must_exist=True)
    if not dest_dir or not os.path.isdir(dest_dir):
        return jsonify({'success': False, 'message': 'Destination folder not found'}), 400
    ...
    if os.path.isdir(source) and is_safe_path(source, final_dest) and os.path.realpath(final_dest) != os.path.realpath(source):
    ...
    try:
        ...
        if server['entry_file'] == os.path.basename(source) and dest_dir_rel:
            db = get_db()
            db.execute("UPDATE servers SET entry_file = ? WHERE id = ?", (os.path.join(dest_dir_rel, os.path.basename(source)), server_id))
            db.commit()
        write_server_log(server_id, 'INFO', f"Moved '{source_rel}' to '{dest_dir_rel or '/'}'")
        return jsonify({'success': True, 'message': f"Moved to {dest_dir_rel or 'root'}"})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/servers/<int:server_id>/files/compress', methods=['POST'])
@login_required
def compress_items(server_id):
    """Create a ZIP archive from selected files/folders inside the server folder."""
    server = check_server_ownership(server_id)
    if not server:
        return jsonify({'success': False, 'message': 'Server not found'}), 404
    ...
    if isinstance(raw_paths, str):
        raw_paths = [raw_paths]
    archive_name = secure_filename((payload.get('name') or '').strip())
    ...
    sources = []
    for rp in raw_paths:
        p = resolve_server_path(server_dir, (rp or '').strip('/'))
        if not p or os.path.realpath(p) == server_root:
            continue
        if os.path.exists(p):
            sources.append(p)
    if not sources:
        return jsonify({'success': False, 'message': 'Select at least one valid file or folder to compress.'}), 400
        
    if not archive_name:
        archive_name = (os.path.basename(sources[0]) if len(sources) == 1 else 'archive')
        archive_name = secure_filename(archive_name)
    if not archive_name.lower().endswith('.zip'):
        archive_name = f"{archive_name}.zip"
    if not archive_name:
        return jsonify({'success': False, 'message': 'Invalid archive name'}), 400
        
    dest = os.path.join(server_root, archive_name)
    if not is_safe_path(server_dir, dest):
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    if os.path.exists(dest):
        return jsonify({'success': False, 'message': 'An archive with that name already exists'}), 400
    ...
    try:
        ...
        write_server_log(server_id, 'INFO', f"Created archive '{archive_name}' ({size_mb:.2f} MB)")
        return jsonify({
            'success': True,
            'message': f"Archive '{archive_name}' created ({size_mb:.2f} MB).",
            'name': archive_name
        })
    except Exception as e:
        if os.path.exists(dest):
...
@app.route('/api/servers/<int:server_id>/files/download-folder', methods=['GET'])
@login_required
def download_folder(server_id):
    """Stream a folder as a ZIP without needing a temp file on disk."""
    server = check_server_ownership(server_id)
    if not server:
        return jsonify({'success': False, 'message': 'Server not found'}), 404
    ...
    server_dir = get_server_dir(server['user_id'], server_id)
    target = resolve_server_path(server_dir, folder_path)
    
    if not target or not os.path.isdir(target):
        return jsonify({'success': False, 'message': 'Folder not found'}), 404
        
    folder_name = os.path.basename(target.rstrip(os.sep)) or 'folder'
    root_name = os.path.basename(os.path.realpath(target)) or 'folder'
    buffer = io.BytesIO()
    try:
        with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
            for full in iter_files_recursive(target):
                arc = os.path.relpath(full, target).replace('\\', '/')
                if folder_path:
                    arc = f"{root_name}/{arc}"
                zf.write(full, arc)
        buffer.seek(0)
    except Exception as e:
        return jsonify({'success': False, 'message': f'Failed to build archive: {str(e)}'}), 500
    
    data = buffer.getvalue()
    buffer.close()
    write_server_log(server_id, 'INFO', f"Downloaded folder '{folder_path or '/'}' as ZIP")
    return send_file(
        io.BytesIO(data),
...
def unzip_file(server_id):
    ...
    payload = get_payload()
    zip_path = (payload.get('path') or '').strip('/')
    dest_rel = (payload.get('dest') or '').strip('/')
    keep_archive = str(payload.get('keep_archive', '')).lower() in ('1', 'true', 'yes')
    server_dir = get_server_dir(server['user_id'], server_id)
    target_zip = resolve_server_path(server_dir, zip_path)
    
    if not target_zip or not os.path.isfile(target_zip):
        return jsonify({'success': False, 'message': 'ZIP file not found'}), 404
    if not zipfile.is_zipfile(target_zip):
        return jsonify({'success': False, 'message': 'Only ZIP archives can be extracted here'}), 400
    
    if dest_rel:
        # "Extract into Folder…" creates the destination on demand
        extract_dir = resolve_server_path(server_dir, dest_rel, must_exist=False)
        if not extract_dir:
            return jsonify({'success': False, 'message': 'Access denied'}), 403
        try:
            os.makedirs(extract_dir, exist_ok=True)
        except Exception as e:
            return jsonify({'success': False, 'message': f'Could not create destination folder: {str(e)}'}), 400
    else:
        extract_dir = os.path.dirname(target_zip)
    
    try:
        with zipfile.ZipFile(target_zip, 'r') as zf:
            validate_zip_members(zf, server_dir, extract_dir)
...
def search_files(server_id):
    ...
    if len(query) < 1:
        return jsonify({'success': True, 'results': [], 'truncated': False})
        
    server_dir = get_server_dir(server['user_id'], server_id)
    root = resolve_server_path(server_dir, scope, must_exist=True)
...
def server_startup(server_id):
    ...
    if request.method == 'POST':
        ...
        auto_restart = 1 if request.form.get('auto_restart') in ('on', 'true', '1', 'yes') else 0
